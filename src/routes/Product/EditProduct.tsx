...
export default function EditProduct({
  ...
  const [originalImage, setOriginalImage] = useState<string>(imageDefault);
  ...
  useEffect(() => {
    ...
    fetchImage(product.image.id as string).then((base64Image) => {
      changeImage(base64Image as string);
      setOriginalImage(base64Image as string);
      reset(data);
    });
  }, [reset, data, product.image.id]);
  ...
  const onSubmit: SubmitHandler<FormField> = async (data) => {
    try {
      ...
      updateProduct(product._id as string, formData);
      const message = await fetchProduct(product._id as string).then(
        (res) => res,
      );
          ...
          <form>
            <div className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                <Label
                  htmlFor="image"
                  className="text-xl font-semibold capitalize underline"
                >
                  Update Image
                </Label>
